...
public class MessagingManagerThread extends Thread {
    ...
    private void handleIncomingMessage(final IncomingMessage incoming) {
        ...
        try {
            ...
            /**
             * TODO STFAN BJDODSON KANAKB See FriendAcceptObj.handleUnprocessed
             * as template, code is something like: if
             * (!mPublicKeyDirectory.verify(in_obj.getString("email"),
             * in_obj.getPublicKey())) {
             * Log.w("Spammer trying to claim public key for email address");
             * return; } if (inAddressBook(email)) { // auto-accept and notify
             * of new friend } else { // notification to accept friend }
             */
    ...
    private class OutgoingMsg implements OutgoingMessage {
        ...
        protected OutgoingMsg(Cursor objs) {
            ...
            if (to != null) {
                ...
                recipientIds = new ArrayList<Long>(subs.getCount());
                subs.moveToFirst();
